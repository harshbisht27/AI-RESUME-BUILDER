      ...
      <Link to={'/dashboard/resume/' + resume.documentId + "/edit"}>
        <div className='p-14 bg-gradient-to-b from-pink-100 via-purple-200 to-blue-200 h-[280px] rounded-t-lg border-t-4'
          ...
          <div className='flex items-center justify-center h-[180px]'>
            {/* Use a proper image URL or fallback */}
            <img src="/cv.png" width={80} height={80} alt="Resume icon" />
          </div>
        </div>
      </Link>

      <div className='border p-3 flex justify-between items-center text-white rounded-b-lg shadow-lg bg-gradient-to-r transition-all'
        style={{
          background: resume?.themeColor || '#000',
        }}>
        <h2 className='text-sm font-medium'>{resume?.title || 'Untitled Resume'}</h2>

        <DropdownMenu>
          <DropdownMenuTrigger>
            <MoreVertical className='h-4 w-4 cursor-pointer hover:text-gray-200 transition-all' />
          </DropdownMenuTrigger>
          <DropdownMenuContent>
